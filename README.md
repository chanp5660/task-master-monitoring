...
- 🔗 **의존성 추적**: 작업 간 연관관계 관리
- 📝 **상세 정보**: 설명, 테스트 전략 등
- 🎯 **드래그앤드롭**: 칸반 보드 스타일 작업 이동

### 🍅 **뽀모도로 타이머**
- ⏰ **집중 타이머**: 25분 집중 / 5분 휴식 사이클
- 📊 **세션 통계**: 일별/월별 생산성 분석
- 🎯 **목표 추적**: 완료율 및 집중 시간 측정
- 📈 **진행률 표시**: 실시간 생산성 대시보드
- 🗂️ **기록 관리**: 세션 기록 저장 및 삭제
...
│   │   ├── PomodoroTimer.jsx       # 뽀모도로 타이머 컴포넌트
│   │   ├── PomodoroStatsModal.jsx  # 뽀모도로 통계 모달 컴포넌트
...
└── projects/                       # 사용자 프로젝트 데이터
    ...
    │   ├── dashboard-memo.json     # 대시보드 메모
    │   ├── pomodoro-history.json   # 뽀모도로 세션 기록
...
### 뽀모도로 타이머
- **POST** `/api/save-pomodoro-session` - 뽀모도로 세션 저장
- **GET** `/api/load-pomodoro-history/:project` - 뽀모도로 기록 로드
- **DELETE** `/api/delete-pomodoro-session` - 뽀모도로 세션 삭제
- **DELETE** `/api/delete-multiple-pomodoro-sessions` - 다중 세션 삭제
...
### 타이머 사용법
1. **타이머 시작**: 헤더의 뽀모도로 타이머 아이콘 클릭
2. **집중 세션**: 25분 집중 작업 (빨간색 표시)
3. **휴식 세션**: 5분 휴식 시간 (초록색 표시)
4. **자동 전환**: 세션 종료 시 자동으로 다음 단계로 이동

### 통계 및 분석
- **📊 통계 보기**: 타이머 옆 통계 버튼 클릭으로 상세 분석 확인
- **일별 기록**: 오늘의 집중 시간, 완료율, 세션 수 확인
- **월별 통계**: 장기간 생산성 패턴 분석
- **세션 관리**: 개별 세션 기록 편집 및 삭제 가능

### 생산성 향상 팁
- **일관된 사용**: 매일 정해진 시간에 뽀모도로 세션 시작
- **목표 설정**: 하루 목표 세션 수 설정하여 동기부여
- **휴식 중요성**: 짧은 휴식으로 집중력 회복
- **기록 분석**: 주기적으로 통계를 확인하여 패턴 파악
...
#### Q: 뽀모도로 타이머는 어떻게 사용하나요?
A: 헤더의 타이머 아이콘을 클릭해서 시작할 수 있습니다. 25분 집중 후 5분 휴식이 자동으로 순환되며, 모든 세션이 기록됩니다.

#### Q: 타이머 소리나 알림을 끌 수 있나요?
A: 현재는 자동 알림이 기본 설정입니다. 브라우저 알림 설정에서 사이트별 알림을 조절할 수 있습니다.

#### Q: 세션 기록은 어디에 저장되나요?
A: 뽀모도로 세션 기록은 프로젝트 폴더 내 `pomodoro-history.json` 파일에 저장됩니다. 통계 버튼을 클릭해서 언제든 확인할 수 있습니다.

#### Q: 실수로 타이머를 중단했는데 기록에 남나요?
A: 완료되지 않은 세션은 기록에 저장되지 않습니다. 다시 타이머를 시작해서 전체 세션을 완료해야 통계에 반영됩니다.
...
### 🚀 최신 릴리즈 (v1.5.3)
- **🍅 뽀모도로 타이머 시스템**: 집중력 향상을 위한 완전한 뽀모도로 타이머 기능 추가
  - 25분 집중 / 5분 휴식 사이클 자동 관리
  - 작업 세션 기록 및 통계 추적
  - 일별/월별 생산성 분석 대시보드
  - 세션 기록 다중 선택 삭제 기능
- **🎨 UI/UX 혁신적 개선**: 
  - 카드 뷰를 칸반 보드 스타일로 완전 리뉴얼
  - 드래그앤드롭 기반 직관적 작업 관리
  - 리사이저블 사이드바로 상세보기 개선
  - 헤더 레이아웃 및 버튼 가시성 최적화
- **📊 생산성 측정 도구**: 실시간 집중 시간 및 완료율 추적
- **🔧 패키지 관리**: npm 패키지 버전 1.5.3 업데이트 및 배포 준비
